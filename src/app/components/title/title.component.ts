...
export class TitleComponent implements OnInit {
    ...
    if (isPlatformBrowser(this.platformId)) {
      setTimeout(() => {
        ...
        if (this.direction === 'right') {
          if (this.inverse) {
            this.data = {value: 'right', params: {width: this.lineWidth, width2: Math.round(
              this.lineWidth * 0.2
            ), width8: Math.round(this.lineWidth * 0.8)}};
          } else {
            this.data = {value: 'left', params: {width: this.lineWidth, width2: Math.round(
              this.lineWidth * 0.2
            ), width8: Math.round(this.lineWidth * 0.8)}};
          }
          this.direction = 'left';
  ...
  ngOnInit() {
    this.data = {value: this.inverse ? 'right' : 'left', params: {width: this.lineWidth, width2: Math.round(
      this.lineWidth * 0.2
    ), width8: Math.round(this.lineWidth * 0.8)}};
  }
}
